...
## 2026-03-06 — Feature 13: Módulo Salão (Atendimento Presencial)

**Decision:** Separate `RestaurantTable` model (distinct from existing `DineInTable`), session-based comanda system with soft-delete items, flexible stock control via `StockMode` enum, native pointer drag-and-drop for table map
**Reason:** The existing `DineInTable` model serves the QR-code digital waiter flow (Feature 5) — a self-service ordering system. The Salão module addresses a fundamentally different use case: staff-managed in-person service with physical table maps, manual order entry (comandas), split billing, and thermal receipt printing. Using a separate `RestaurantTable` model avoids breaking the existing QR flow and keeps concerns cleanly separated. Sessions (`TableSession`) can optionally reference a table or be table-less (for counter/balcão service via `guestName`). Items use soft-delete (`removedAt`) instead of hard delete so that stock can be properly restored when items are removed — the `addItem`/`removeItem`/`updateItemQty` functions all run in `$transaction` to prevent stock race conditions. The `StockMode` enum (NONE/AVAILABLE/BY_QUANTITY) provides three flexibility levels: no control for most items, simple available/unavailable toggle for items like daily specials, and precise quantity tracking for premium/limited items. Auto-pause (setting `isAvailable=false` when `stockQty` hits 0) prevents overselling. The table map uses native `PointerEvent` handlers instead of `@dnd-kit` for actual dragging — simpler, no extra runtime cost for the drag interaction itself (`@dnd-kit` deps added but the map uses pointer capture for smoother native drag). Split billing uses in-memory `Math.ceil` division to ensure the restaurant never loses a centavo on rounding.
**Impact:** New `StockMode`/`SessionStatus` enums, `RestaurantTable`/`TableSession`/`SessionItem` models. `stockMode`/`stockQty` fields on MenuItem (alongside existing `trackStock`/`stockQuantity` for backward compatibility). `hasTableManagement` plan flag on Plan (Profissional+). 13 new API endpoints. 5 new frontend components (SalaoPage + 4 modals). Stock popover added to MenuPage. Nav item with Armchair icon.
